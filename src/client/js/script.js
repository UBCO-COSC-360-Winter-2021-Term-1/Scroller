$(document).ready(() => {
	...
	/* Sort Posts by Top Votes In Thread*/
	$("#top-posts-sort").click((e) => {
		e.preventDefault();
		var threadUrl = window.location.pathname.split("/")[2];
		var sortType = $("#top-posts-sort").text().trim();
					...
					$.each(result, (_, element) => {
						...
						if (element["comments"].length > 0) {
							element['comments'].forEach((comment) => {
								...
								result += `<button id="hide" class="me-4 thread-hide">Hide</button>`
								result += `<button id="delete" class="thread-delete">Delete</button>`
								result += `</div>`
								result += `</div>`
								result += `</div>`
								result += `</article>`;
		...
		/* Sort Posts by Newest Release In Thread*/
		$("#new-posts-sort").click((e) => {
			e.preventDefault();
			var threadUrl = window.location.pathname.split("/")[2];
			var sortType = $("#new-posts-sort").text().trim();
			$.ajax({
				url: `http://${$(location).attr('host')}/server/middlewares/PostMiddleware.class.php`,
				dataType: "json",
				contentType: "application/json;charset=utf-8",
